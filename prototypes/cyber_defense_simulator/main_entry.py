...
def parse_args():
    ...
    parser.add_argument(
        "--train",
        action="store_true",
        help="Train RL agent before running simulation"
    )
...
def main():
    ...
    if args.quick_test:
        args.episodes = 5
        logger.info("Running in quick test mode (5 episodes total, training happens automatically in backend)")
    
    # Training happens automatically in backend - no need for flags
    # But keep --no-train for special cases
    ...
    try:
        ...
        # Run simulation (training happens automatically in backend)
        logger.info(f"Starting simulation with {args.episodes} total episodes")
        logger.info("Note: Training happens automatically in backend (20% of episodes, min 3, max 20)")
        if attack_types:
            logger.info(f"Attack types: {[at.value for at in attack_types]}")
        
        # Override training if --no-train is specified
        if skip_training:
            # Temporarily override to skip training
            original_run = orchestrator.run_simulation
            def run_without_training(num_episodes=None, attack_types=None):
                # Run episodes directly without training
                if num_episodes is None:
                    num_episodes = Config.NUM_EPISODES
                orchestrator.metrics = SimulationMetrics(
                    total_episodes=0, successful_defenses=0, failed_defenses=0,
                    false_positives=0, average_reward=0.0,
                    average_time_to_remediate=0.0, detection_rate=0.0
                )
                orchestrator.episodes = []
                logger.info(f"Starting simulation without training: {num_episodes} episodes")
                for i in range(num_episodes):
                    attack_type = None
                    if attack_types:
                        attack_type = attack_types[i % len(attack_types)]
                    episode = orchestrator.run_episode(i + 1, attack_type)
                    orchestrator.rl_agent.decay_epsilon()
                    if (i + 1) % 10 == 0:
                        orchestrator._log_progress()
                orchestrator._log_final_metrics()
                return orchestrator.metrics
            orchestrator.run_simulation = run_without_training
